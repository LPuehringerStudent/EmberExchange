...
import { PlayerStatisticsService, type PlayerStatistics } from '@core/services/player-statistics.service';
import { DailyStatisticsService, type DailyStatistics, type DailySummary } from '@core/services/daily-statistics.service';
...
export class StatisticsComponent implements OnInit {
  ...
  private readonly _playerStatsService = inject(PlayerStatisticsService);
  private readonly _dailyStatsService = inject(DailyStatisticsService);
  private readonly _stoveTypeStatsService = inject(StoveTypeStatisticsService);
  ...
  async loadAllData() {
    ...
    const promises: Promise<any>[] = [
      firstValueFrom(this._playerStatsService.getAllPlayerStatistics()),
      firstValueFrom(this._playerStatsService.getTopPlayersByActivity(5)),
      firstValueFrom(this._playerStatsService.getTopPlayersByNetWorth(5)),
      firstValueFrom(this._dailyStatsService.getTodayStatistics()),
      firstValueFrom(this._dailyStatsService.getDailySummary(7)),
