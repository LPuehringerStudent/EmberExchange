...
describe("WebSocket Integration", () => {
    ...
    beforeAll(async () => {
        ...
        try {
            ...
            const gameStmt = unit.prepare<unknown, { gameType: string; name: string; slug: string; minPlayers: number; maxPlayers: number }>(
                `INSERT INTO Game (gameType, name, slug, minPlayers, maxPlayers, ruleset, description, genre, tags, isActive)
                 VALUES (@gameType, @name, @slug, @minPlayers, @maxPlayers, '', '', '', '[]', 1)
                 ON CONFLICT (gameType) DO NOTHING`,
                { gameType: "poker", name: "Poker", slug: "poker", minPlayers: 2, maxPlayers: 6 }
            );
            await gameStmt.run();
            const gameStmt2 = unit.prepare<unknown, { gameType: string; name: string; slug: string; minPlayers: number; maxPlayers: number }>(
                `INSERT INTO Game (gameType, name, slug, minPlayers, maxPlayers, ruleset, description, genre, tags, isActive)
                 VALUES (@gameType, @name, @slug, @minPlayers, @maxPlayers, '', '', '', '[]', 1)
                 ON CONFLICT (gameType) DO NOTHING`,
                { gameType: "blackjack", name: "Blackjack", slug: "blackjack", minPlayers: 1, maxPlayers: 5 }
            );
            await gameStmt2.run();
