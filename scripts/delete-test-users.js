/**
 * Delete all player accounts whose username starts with "test_".
 *
...
async function main() {
    ...
    try {
        ...
        const countRes = await client.query(
            `SELECT COUNT(*) AS cnt FROM Player WHERE username LIKE 'test_%'`
        );
        const total = parseInt(countRes.rows[0].cnt, 10);
        console.log(`Found ${total} accounts starting with "test_"`);
        ...
        // 2. Create a temp table with the target IDs for fast joining
        await client.query(`
            CREATE TEMP TABLE _test_user_ids ON COMMIT DROP AS
            SELECT playerId FROM Player WHERE username LIKE 'test_%'
        `);
        await client.query(`CREATE INDEX ON _test_user_ids (playerId)`);

        // 3. Delete dependent records in dependency order

        // Trade references Listing and Player
        const tradeBuyerRes = await client.query(`
            DELETE FROM Trade WHERE buyerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${tradeBuyerRes.rowCount} Trade records (test_ as buyer)`);

        const tradeSellerRes = await client.query(`
            DELETE FROM Trade WHERE listingId IN (
                SELECT listingId FROM Listing WHERE sellerId IN (SELECT playerId FROM _test_user_ids)
            )
        `);
        console.log(`  Deleted ${tradeSellerRes.rowCount} Trade records (test_ as seller)`);

        // Listing references Player, Stove, Lootbox
        const listingRes = await client.query(`
            DELETE FROM Listing WHERE sellerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const lootboxDropRes = await client.query(`
            ...
            WHERE lootboxId IN (
                SELECT lootboxId FROM Lootbox WHERE playerId IN (SELECT playerId FROM _test_user_ids)
            )
        `);
        ...
        const ownershipRes = await client.query(`
            DELETE FROM Ownership WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const gloryShowcaseRes = await client.query(`
            DELETE FROM GloryShowcase WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const gloryFeatRes = await client.query(`
            DELETE FROM GloryFeaturedAchievement WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const gloryGbRes = await client.query(`
            ...
            WHERE playerId IN (SELECT playerId FROM _test_user_ids)
               OR authorId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const gloryVisitRes = await client.query(`
            ...
            WHERE visitorPlayerId IN (SELECT playerId FROM _test_user_ids)
               OR visitedPlayerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const stoveRes = await client.query(`
            DELETE FROM Stove WHERE currentOwnerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${stoveRes.rowCount} Stove records`);

        // Lootbox references Player
        const lootboxRes = await client.query(`
            DELETE FROM Lootbox WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const shopPurchaseRes = await client.query(`
            DELETE FROM ShopPurchase WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const roomPlayerRes = await client.query(`
            DELETE FROM RoomPlayer WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const chatRes = await client.query(`
            ...
            WHERE senderId IN (SELECT playerId FROM _test_user_ids)
               OR receiverId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const coinRes = await client.query(`
            DELETE FROM CoinTransaction WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const loginRes = await client.query(`
            DELETE FROM LoginHistory WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const miniGameRes = await client.query(`
            DELETE FROM MiniGameSession WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const dailyRewardRes = await client.query(`
            DELETE FROM PlayerDailyReward WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const gloryBannerRes = await client.query(`
            DELETE FROM PlayerGloryBanner WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${gloryBannerRes.rowCount} PlayerGloryBanner records`);

        const gloryThemeRes = await client.query(`
            DELETE FROM PlayerGloryTheme WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${gloryThemeRes.rowCount} PlayerGloryTheme records`);

        const gloryTitleRes = await client.query(`
            DELETE FROM PlayerGloryTitle WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${gloryTitleRes.rowCount} PlayerGloryTitle records`);

        const gloryTrophyRes = await client.query(`
            DELETE FROM PlayerGloryTrophy WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const prestigeRes = await client.query(`
            DELETE FROM PlayerPrestige WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const prestigeLogRes = await client.query(`
            DELETE FROM PrestigeLog WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${prestigeLogRes.rowCount} PrestigeLog records`);

        // PlayerStatistics references Player
        const statsRes = await client.query(`
            DELETE FROM PlayerStatistics WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${statsRes.rowCount} PlayerStatistics records`);

        // Session references Player
        const sessionRes = await client.query(`
            DELETE FROM Session WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${sessionRes.rowCount} Session records`);

        // SupportTicket references Player
        const ticketRes = await client.query(`
            DELETE FROM SupportTicket WHERE reporterId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const tfBackupRes = await client.query(`
            DELETE FROM TwoFactorBackupCode WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${tfBackupRes.rowCount} TwoFactorBackupCode records`);

        const tfChallengeRes = await client.query(`
            DELETE FROM TwoFactorChallenge WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const pityRes = await client.query(`
            DELETE FROM PlayerPity WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${pityRes.rowCount} PlayerPity records`);

        // PlayerQuest references Player
        const questRes = await client.query(`
            DELETE FROM PlayerQuest WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const settingsRes = await client.query(`
            DELETE FROM PlayerSettings WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        ...
        const eventRes = await client.query(`
            DELETE FROM EventLog WHERE playerId IN (SELECT playerId FROM _test_user_ids)
        `);
        console.log(`  Deleted ${eventRes.rowCount} EventLog records`);

        // 4. Finally delete the players themselves.
        const playerRes = await client.query(`
            DELETE FROM Player WHERE username LIKE 'test_%'
        `);
        console.log(`\n✅ Deleted ${playerRes.rowCount} test accounts from Player.`);
        ...
        await client.query("ROLLBACK");
        console.error("❌ Failed to delete test users — transaction rolled back:", err);
        process.exit(1);
    } finally {
        client.release();
        await pool.end();
