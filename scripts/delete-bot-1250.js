/**
 * Deletes already-banned bot account 1250 (gw6767) and all related data.
 */
...
async function main() {
    ...
    try {
        ...
        const targetRes = await client.query(
            `SELECT playerId, username, email, coins, bannedAt, banReason FROM Player WHERE playerId = $1`,
            [BOT_ID]
        );
        ...
        console.log(`🎯 Deleting banned bot: ${bot.username} (ID: ${bot.playerid}, Coins: ${bot.coins}, Banned: ${bot.bannedat}, Reason: ${bot.banreason})`);
        ...
        console.log(`\n✅ Deleted ${playerRes.rowCount} bot account: ${playerRes.rows[0]?.username || 'unknown'}`);
